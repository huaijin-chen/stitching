...
vector<int> MyRandGen(const vector<FeatureMatch> &matches)
{
...
int MyRandSelectInliers(const vector<int>& vrandNums, int offPos,
					  ...
					  int nRANSAC, double RANSACthresh, CTransform3x3& M)
{
	...
	set<int> visitedPair;
	vector<int> vRandNums = MyRandGen(matches);
	int offPos = 0;

	for(int i = 0; i < nRANSAC; i++)
	{
		vector<int> inliers;
		offPos = MyRandSelectInliers(vRandNums, offPos, matches, inliers, visitedPair);
		if(inliers.size() < 2)
			break;

		CTransform3x3 Mtmp;
		MyleastSquaresFit(f1, f2, matches, m, f, WH1, WH2, inliers, Mtmp);

		MycountInliers(f1,f2,matches,m,f,WH1,WH2,Mtmp,RANSACthresh,inliers);
	...
	MyleastSquaresFit(f1, f2, matches, m,f,WH1,WH2,vInliers[maxPos],M);
	// END TODO

	return vInliers[maxPos];
}

void MyPrintVector(const CVector3 &v)
{
	cout<<endl;
				 ...
				 CTransform3x3 M, double RANSACthresh, vector<int> &inliers)
{
	...
	for (unsigned int i=0; i<f1.size(); i++) {
		...
		float cx1 = WH1.first / 2.0f, cy1 = WH1.second / 2.0f;
		float cx2 = WH2.first / 2.0f, cy2 = WH2.second / 2.0f;
		const float x1 = (float)f1[i].x,	y1 = (float)f1[i].y;
		int f2pos = matches[i].id - 1;
		const float x2 = (float)f2[f2pos].x,	y2 = (float)f2[f2pos].y;
		...
		//q = MP, so q[0]=(x2'-cx2)/f/weight'
		q = M * p;
				
		float weight = sqrt((pow(f,2)+pow(x2-cx2,2)+pow(y2-cy2,2))/(pow(f,2)+pow(x1-cx1,2)+pow(y1-cy1,2)));
		p[0] = (x2-cx2)/f/weight;
		p[1] = (y2-cy2)/f/weight;
		double dist = sqrt(pow(p[0]-q[0],2)+pow(p[1]-q[1],2))*f;
...
void MyNormRotationMat(CTransform3x3 &M)
{
	CVector3 p,q,r,vtmp;
...
void MyPrintMat(const CTransform3x3 &M)
{
	cout<<endl;
					...
					const vector<int> &inliers, CTransform3x3& M)
{
	...
	for(size_t i = 0; i < inliers.size(); i++)
	{
		...
		float cx1 = WH1.first / 2.0f, cy1 = WH1.second / 2.0f;
		float cx2 = WH2.first / 2.0f, cy2 = WH2.second / 2.0f;
		const float x1 = (float)f1[f1pos].x, y1 = (float)f1[f1pos].y;
		const float x2 = (float)f2[f2pos].x, y2 = (float)f2[f2pos].y;
		
		float weight = sqrt((pow(f,2)+pow(x2-cx2,2)+pow(y2-cy2,2))/(pow(f,2)+pow(x1-cx1,2)+pow(y1-cy1,2)));
		
		v1[0] = (x1 - cx1) * weight / f;	
		v1[1] = (y1 - cy1) * weight / f;	
		v1[2] = weight;
