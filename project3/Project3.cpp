...
//
// NAME
//  Project3.cpp -- command-line (shell) interface to project 2 code
//
// SYNOPSIS
//  Project3 rdWarp input.tga output.tga f k1 k2
//  Project3 alignAll images.txt orientations.txt minMatches f nRANSAC RANSACthresh [sift]
//  Project3 blendAll orientations.txt outfile.tga f blendRadius
//  Project3 script script.cmd
//
// PARAMTERS
...
int RDWarp(int argc, const char *argv[])
{
    ...
    CShape sh = src.Shape();
    CFloatImage uv = WarpRDField(sh, sh, f, k1, k2);
...
int AlignAll(int argc, const char *argv[])
{
    ...
    for (n = 0; fgets(line, 1024, stream); n++)
    {
		...
		WH.push_back(make_pair(width,height));
    }
	...
	if (initGlobalAlign(fs, minMatches, eRotate3D, f, WH, nRANSAC, RANSACthresh, am, ms) == -1)
		throw CError("%s: image graph not connected", argv[1]);

	// Do bundle adjustment.
	bundleAdjust(fs, eRotate3D, f, WH, am, ms, bundleAdjustIters);
...
int BlendAll(int argc, const char *argv[])
{
    ...
    CByteImage result = BlendImages(ipList, f, blendRadius);
...
void ParsingIni()
{
	Match_threshold_for_ratio = 1.0;
	CenterImageIndex = 0;
	ApproximateRotation = 1;

	ifstream infile("config.ini");
	if(!infile)
	{
		cout<<"Can't open config.ini. We'll use default parameters."<<endl;
		return;
	}

	string name;
	infile>>name;
	if(name == "Match_threshold_for_ratio=")
		infile>>Match_threshold_for_ratio;
	
	infile>>name;
	if(name == "CenterImageIndex=")
		infile>>CenterImageIndex;

	infile>>name;
	if (name == "ApproximateRotation=")
		infile>>ApproximateRotation;
